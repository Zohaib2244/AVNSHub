...
**Interaction model**: widgets are **resizable** (S/M/L × h/v) and **rearrangeable** (drag in edit mode); a widget shows more when bigger by **rendering different markup per size**. A prior hover/click/grow-expansion subsystem (flyout/overlay) was removed because it caused a hover-oscillation loop — framer-motion `layout` and dnd-kit's `animateLayoutChanges` both re-measure + setState per item after a reflow, and dense grid flow moves *every* item, so either one loops into "Maximum update depth exceeded". If hover/expand interaction is reintroduced, design around that failure mode: no layout-animation systems on grid items, and no state changes triggered from hover/measurement callbacks that themselves cause a reflow.
