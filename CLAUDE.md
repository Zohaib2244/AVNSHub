...
| Text muted (content) | `#b5b0a3` | `#514739` |
| Text muted-dim (labels) | `#8b8576` | `#796856` |
| Orange accent | `#ff6b2b` | `#e05a18` |
| Cyan/teal accent | `#00b4c8` | `#00768a` |

- **Text colour is a role, not a shade**: `--text-muted` is *content* (sub text, values, prose) and `--text-muted-dim` is *labels* (the uppercase DotGothic section/field headers). Every palette solves them to **8.0:1** and **4.7:1** against `--bg-card`, a fixed 1.70:1 gap, so a header never reads as the same tone as the text under it - which it did when `.block-label` and `.block-sub` both used `--text-muted`. Interactive elements (`.seg-btn`, `.palette-btn`, `.github-tab`, tabs and dialog buttons) deliberately stay on `--text-muted`: their muted state is the resting half of a hover-to-accent transition, not a label. Any new palette must hit both targets in dark *and* light - the eight light modes previously sat at 2.9-3.5:1 and 2.0-2.2:1, failing WCAG AA outright.
...
- **Typography**: sizes come from the **five-step type scale** defined in `styles/globals.css` (`--fs-micro` 0.65rem / `--fs-label` 0.75rem / `--fs-body` 0.875rem / `--fs-lead` 1.2rem / `--fs-stat` 2.2rem; root is 17px). Every `font-size` in the codebase references one of these - never reintroduce ad-hoc rem values. Each step is >=1.15x the last and floored at ~11px, the legibility limit of DotGothic16 (a bitmap-grid face that drops strokes below it). Size alone cannot carry hierarchy at these sizes, so pair the step with family and colour: `DotGothic16` + uppercase + `letter-spacing: 0.14em` for section/field labels (`--fs-label`) and headline stat numbers (`--fs-stat`, `line-height: 1` - the largest text on the page); `JetBrains Mono` for primary data values (`--fs-lead`, `font-weight: 500`), sub text and prose (`--fs-body`), and chips/pills/badges (`--fs-micro`, uppercase). The namecard logo (`1.7rem`) stays outside the scale - it is a brand mark, not text hierarchy. The NutBot terminal/chat/widget-creator surfaces keep their own independent `calc(<rem> * var(--nb-scale))` scale and are deliberately not on these tokens.
...
**How S/M/L is picked (Slot Layout)**: automatically from the cell's measured pixel box (`lib/grid/sizeClass.ts` — width and height must *each* clear `SIZE_CLASS_PX`), unless the user pins one with the `layoutSize` framework setting (`auto | S | M | L` in `FRAMEWORK_SETTINGS`, the widget's gear › *layout size*). The popover offers only the sizes the manifest declares and hides the field for single-size widgets; `SlotWidgetCell` applies a pin over the box-derived size (including the fill-the-region largest-variant rule) and ignores a stale pin for a size the widget no longer declares. Orientation always comes from the box. A pinned L in a small cell crops — widgets must clip cleanly (see `docs/CREATING_WIDGETS.md` § Sizing).
...
**AVN Hub Canvases** (`lib/canvases.ts`, `components/dashboard/CanvasSwitcher.tsx`): Arc-spaces-style named layouts — one canvas per context (e.g. home / work / entertainment). Rendered as a horizontal pill row at the left end of Hub Core's control deck, inside `HubCorePanel`'s `.hub-core-bar` and separated from the view|edit switch by `.hub-core-divider` â€” not a separate UI surface. Each pill shows its glyph and its name side by side (the label is simply visible now; the old vertical `writing-mode: vertical-rl` hover-expand belonged to the narrow right edge and is gone, as is the collapse chevron â€” Hub Core hides itself wholesale instead). A long name is clamped with `max-width` + ellipsis so one canvas can't stretch the deck past the frame. Double-click a pill to open a rename/delete flyout (`.canvas-manage-panel`, opens upward out of the pill, reuses the single shared `activePopover` from `LayoutProvider`). Canvas identity (`{ id, name, icon? }` list + `activeId`) persists to `localStorage["nutmag-canvases"]`; `icon` is optional, one of `config/canvasIcons.ts`'s curated Lucide set (`CanvasIconPicker` for picking one, `CanvasGlyph` for rendering it or falling back to the first-letter-of-name glyph). Both the widget arrangement (`lib/slotLayout.ts`) and theme mode + palette (`lib/theme.ts`) are per-canvas, namespaced via `canvasScopedKey(baseKey, canvasId)` (the bare key for the original `default` canvas — no migration needed for existing users; `<key>::<id>` for every canvas created afterward). New canvases start with an empty layout and the hardcoded theme defaults, not a clone of `default`. Both `slotLayout.ts` and `theme.ts` independently `subscribeCanvases()` at module load and re-apply/notify their own listeners on an actual id change, so neither module — nor `CanvasSwitcher` itself — needs to know about the other; `app/layout.tsx`'s pre-paint script duplicates the same `canvasScopedKey` logic inline (can't import TS there) to avoid a flash of the wrong canvas's theme. Deleting a canvas removes its slot-layout/theme/palette keys too; the last remaining canvas can't be deleted. Management UI exists in two places sharing the same mutators (`lib/canvases.ts`): a double-click flyout on the pill itself (`.canvas-manage-panel`, opens upward, reuses the single shared `activePopover` from `LayoutProvider`), and a dedicated "canvases" accordion section in Hub Core Settings (`CanvasesSettings`/`CanvasSettingsRow`) for full at-a-glance rename/icon/delete/add. Note: `.canvas-pill-list` deliberately has no `overflow: hidden` (it would clip the flyout, which is an absolutely-positioned descendant escaping upward out of the deck) — see the comment in `globals.css`.
...
**Hub Core canvas settings**: theme, palette, global prefs, layout reset, region grid-size controls, and layout import/export live in the Hub Core settings tab. Settings are a **section rail**, not an accordion: six sections down the left (`theme`, `canvas`, `widget style`, `layout`, `canvases`, `system`, listed in `config/hubSettings.ts`) and exactly one section's body on the right. The old accordion stacked four headers plus an open body into one 250px column, with "appearance" alone carrying six unrelated groups; it is now split by what each control acts on (the two backdrop dials in particular sit in different sections, because the widget one is a global default that does *not* cascade from the canvas one), and the layout reset lives in `layout` next to the layout it throws out. The selected section is held in `LayoutProvider` (`settingsSection`), not the panel, so the command palette can open straight to one. Switching sections tweens the panel's height (`AutoHeight` in `HubCorePanel`): a ResizeObserver measures `.hub-settings-scroll` and framer animates `height` — not the `layout` prop, whose scale transform squashes text mid-resize. It must measure the scroller (already floored by the rail and capped by the max-height), not the raw section content, or most of the tween runs through heights the panel clamps away and the visible part still snaps; so the cap and the scrolling live on `.hub-settings-scroll`, and `.hub-core-panel-settings` neither caps nor scrolls itself. Below 640px the rail becomes a wrapping chip row above the body. Edit mode is controlled by the deck's view|edit switch (and its command-palette entry), not by duplicated controls inside the settings panel.

**Hub Core is the control deck** (`components/dashboard/HubCorePanel.tsx`, `lib/useDeckAutoHide.ts`): one horizontal sticker bar centred on the bottom of the **canvas** â€” not the window â€” holding the canvas switcher, a two-state view|edit switch, canvas settings, the widget manager and snapshot. It replaced the right-edge tab column. Panels open **upward** out of the bar and are centred on it (`bottom`, not `top`; bar, stub and panel are all centred with `left/right: 0` + `margin-inline: auto` rather than `left: 50%` + `translateX(-50%)` — for an abspos box `left: 50%` also halves the width available to lay out in, which made the bar wrap at half the room it had, and it leaves the transform free for the collapse animation), so the canvas being changed stays visible. It **auto-hides**: at rest it is `.hub-core-stub`, a small sticker of three dots straddling the frame's bottom border â€” one dot per canvas with the active one lit, so a put-away deck still reports where you are â€” and Hub Core therefore costs zero canvas until reached for. There is no squeezed in-between state: a stub, or the whole bar. `.hub-core-deck` itself paints nothing; it is a transparent `pointer-events: none` anchor layer over the frame's padding box so bar, stub and panel can each be centred independently (which is also why the outside-click test is a `closest(".hub-core-bar, .hub-core-panel, .hub-core-stub")` rather than `panelRef.contains` â€” the layer covers every widget). Timings live in `useDeckAutoHide`: 90ms dwell before opening, a 0.5s grace before closing (leaving the deck is usually on the way back to it; Escape still dismisses it at once). The hot zone is the visible thing, not a strip of screen: collapsed, only a 28px margin around the pill arms it; open, the bar's own box plus 18px, so the pointer can travel the whole bar. It used to be the frame's full width across its bottom 92px, which opened the deck whenever the pointer went near the bottom row of widgets. Pinning is OR-ed into the result rather than pushed into state (`open = pinned || hoverOpen || focusOpen`), so an open tab, edit mode or a live popover can never let it collapse mid-task and no effect is needed to re-open it. Focus pins only on `:focus-visible`: a plain `focus` also fires on click, which pinned the deck open permanently after any click in it. Proximity is skipped entirely on coarse pointers â€” there the stub is just a tap target. Prototype, and the rejected `chin`/`float` docking alternatives: `public/proto/control-deck.html`.

**Command palette** (`components/dashboard/CommandPalette.tsx`): `Ctrl/Cmd+K` anywhere, or the key-cap button at the right end of the deck. Mounted by `LayoutProvider` beside `HubDialog` — outside `.frame`, so the canvas's `backdrop-filter` can't become its containing block, and it works whether or not the deck is out. Commands are rebuilt from live state each render: every canvas (via `requestCanvasSwitch`, the same guard the pills use while a widget build is running) plus `new canvas…` (opens the deck's real create flyout through `CREATE_POPOVER_KEY`; the flyout seeds its draft on the popover transition, so both entry points get a default name), edit mode, widget manager, snapshot (`lib/snapshotCanvas.ts`, shared with the deck), each settings section, the three theme modes, every palette pack, layout export, and layout reset **through a `showHubDialog` confirm** — one keystroke is too close to a destructive reset. Filtering is order-free whole-word substring matching over group + label + keywords, never fuzzy enough to surprise. `paletteOpen` lives in `LayoutProvider` and the deck's two Escape handlers stand down while it is true, so one Escape closes one thing; inside the palette the first Escape clears a typed query and the second closes. It deliberately does **not** take the chord inside the NutBot shell (`.term-xterm`, where Ctrl+K is readline's kill-line), while a `HubDialog` is up, or during a widget install. z-index 500: above deck/panels, below the install cover and dialogs. No single-letter shortcuts exist anywhere — they would fire while typing in NutBot chat or a rename field.
...
> - In-UI chat (NutBot's `CREATOR` tab) that scaffolds new widgets on request, backed by a choice of CLI harness â€” `claude`, `codex`, or `opencode` (`lib/widget-creator/harnessAdapters.ts`), with auto-fallback if one hits a rate limit. Generates the content component + manifest entry inside a per-widget **workbench** (a source mirror outside the repo — see [`docs/WIDGET_WORKBENCH.md`](./docs/WIDGET_WORKBENCH.md)), TypeScript-checks it there, and only then applies it to the live tree; a failed draft never touches the running dashboard. Widget import uses the same pipeline. Follows `docs/CREATING_WIDGETS.md` as its own prompt context, so that doc stays authoritative for both humans and the LLM.
...
> - New `chat` tab in NutBot's terminal, alongside `log`/`shell`/`creator` â€” a real conversational personality, not a tool. Setup guide: [`docs/NUTBOT_CHAT_SETUP.md`](./docs/NUTBOT_CHAT_SETUP.md).
> - Dual-backend design: `chatBackend` in `lib/prefs.ts` defaults to `auto`, which prefers self-hosted **Bonfire** (`NUTBOT_CHAT_URL`, proxied through `/api/nutbot-chat`) when reachable, then falls back through the same shared `harnessChain` used by the Widget Creator (`claude`, `codex`, `opencode`). `chatBackend: "off"` disables chat; `creatorEnabled` independently disables only the Widget Creator tab.
> - Bonfire mode keeps the full local-LLM behavior: `lib/nutbot/persona.ts` defines the SFW/NSFW system prompts, the route find-or-creates Bonfire presets, streams Bonfire NDJSON (`token`/`status`/`search_results`/`done`) straight through, and web search is on by default. NSFW/search toggles render only for Bonfire because harness backends do not expose equivalent local orchestration.
> - Harness mode uses `lib/nutbot/chatHarness.ts` to normalize CLI output into the same chat NDJSON frames; Claude uses native `--session-id`/`--resume`, Codex uses `exec resume`, and Opencode currently runs lean/stateless unless its installed CLI exposes compatible continuity. NutBot's face reacts via `lib/nutbotSignal.ts`: `thinking`, `speaking`, `browsing`, and error states. That bus is a priority map, not a single variable — events outrank the ambient mood written by `lib/nutbotMood.ts` (idle time, time of day, machine load), so chatter always wins and the mood is the floor the face falls back to. Per-frame inputs that must never cause a render — streaming token rate for the mouth, pointer position for the gaze, glance targets, machine load — go through `lib/nutbotSense.ts` instead.
...
NUTBOT_CHAT_URL=        # optional - base URL of a self-hosted Bonfire instance
                        # (github.com/shahwaizse/bonfire) behind llama.cpp. Defaults to
                        # http://127.0.0.1:8000. NutBot chat can also fall back to the
                        # shared claude/codex/opencode CLI harness chain, or be turned off.
                        # See docs/NUTBOT_CHAT_SETUP.md.
```
