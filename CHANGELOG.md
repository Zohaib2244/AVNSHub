...
- **Widget Creator now follows a one-way pipeline.** Plan can only hand off to Ideate, Ideate finalizes into Build/Edit, earlier stages reopen as review-only transcripts, built widgets are labeled as Edit mode, and install refreshes return to the same project Build panel.
