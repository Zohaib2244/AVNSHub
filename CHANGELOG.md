...
### Added
- **Visibility-aware animation scheduler.** Canvas widgets share `startAnimationLoop`, which caps frame rates and stops fully off-screen/backgrounded; the authoring guide and harness skill use the same pattern for future widgets.
...
### Changed
- **Always-on client work is substantially lower.** NutBot's L-only terminal/creator tree is lazy-loaded; the default glyph, dot-matrix, Verlet, Spotify, and Wallpaper Engine visualizers are frame-capped with cached palette reads; Ambient Data uses one mosaic animation instead of 64 independently composited cell animations; shared polling pauses in background tabs and de-duplicates overlapping requests.
- **The normal development server uses less memory.** `npm run dev` disables Node source maps, while `npm run dev:debug` keeps the full server-source-map path available for troubleshooting.
...
### Fixed
- **Long-lived resource cleanup.** NutBot's blink-close timeout can no longer restart its timer chain after unmount, abandoned PTY sessions are reaped after five minutes, terminal reconnect scrollback is bounded to about 1 MB, concurrent system-telemetry requests share one in-flight sample, and HMR replaces shared sync pollers instead of retaining duplicate timers and visibility listeners.
- **Windows installer prerequisites are explicit.** `install.bat` now rejects Node.js versions older than the 20.9 minimum before dependency installation, instead of failing later with an opaque Next.js engine error.
