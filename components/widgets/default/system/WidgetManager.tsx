...
//
// Custom widgets (generated by NutBot) also have a permanent delete button
// that removes their files and unregisters them via the delete API route.
//
...
const CUSTOM_IDS = new Set(Object.keys(CUSTOM_WIDGETS));
...
export function WidgetManager() {
  ...
  async function deleteCustomWidget(id: string) {
    if (!CUSTOM_IDS.has(id) || deleting) return;
    setDeleting(id);
    // remove from active layout first so UI cleans up immediately
    if (isSlot) removeSlotWidget(id);
    else updateInstance(id, { hidden: true });
    try {
      ...
      {ids.length === 0 && empty ? (
        ...
        <div className="wm-list">
          {ids.map((id) => {
            ...
            const isCustom = customIds?.has(id) ?? false;
            return (
              <div className="wm-item" key={id}>
                ...
                <div className="wm-item-text">
                  ...
                  <span className="wm-item-id">
                    #{id}
                    {isCustom && <span className="wm-custom-badge">custom</span>}
                  </span>
                ...
                {isCustom && onDelete && (
                  <button
                    type="button"
                    className="wm-btn wm-btn-delete"
                    disabled={deleting === id}
                    onClick={() => onDelete(id)}
                    aria-label={`delete ${manifest.title} widget permanently`}
                    title="delete widget permanently (removes files)"
                  >
